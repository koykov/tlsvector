...
TLS vector это парсер TLS/DTLS пакетов и генератор их отпечатков. Позволяет получить детальные данные пакета и его расширений.
Поддерживает два типа TLS/DTLS пакетов:
* Client Hello
* Server Hello

Также позволяет вычислить `JA3`/`JA4` отпечатки этих пакетов.
...
Для Server Hello пакета, при аналогичных вызовах методов `JA3` и `JA4` будут возвращены `JA3s` и `JA4s` значения.

DTLS пакет парсится и обрабатывается аналогично.
